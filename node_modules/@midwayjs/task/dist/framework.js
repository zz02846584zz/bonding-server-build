...
let TaskFramework = class TaskFramework extends core_1.BaseFramework {
    ...
    getFrameworkType() {
        return decorator_1.MidwayFrameworkType.TASK;
    }
    async run() { }
    async loadTask() {
        const legacyConfig = this.configService.getConfiguration('taskConfig');
        if (legacyConfig) {
            (0, core_2.deprecatedOutput)('[midway:task] Please use "task" replace "taskConfig"');
            this.configService.addObject({
                task: legacyConfig,
            });
        }
        const taskConfig = this.configService.getConfiguration('task');
        const modules = (0, decorator_1.listModule)(decorator_1.MODULE_TASK_KEY);
        for (const module of modules) {
            const rules = (0, decorator_1.getClassMetadata)(decorator_1.MODULE_TASK_METADATA, module);
            for (const rule of rules) {
                ...
                queue.process(async (job) => {
                    ...
                    try {
                        ...
                        await decorator_1.Utils.toAsyncFunction(rule.value.bind(service))(job.data);
                    }
                    catch (e) {
    ...
    async loadLocalTask() {
        const taskConfig = this.configService.getConfiguration('task');
        const modules = (0, decorator_1.listModule)(decorator_1.MODULE_TASK_TASK_LOCAL_KEY);
        for (const module of modules) {
            const rules = (0, decorator_1.getClassMetadata)(decorator_1.MODULE_TASK_TASK_LOCAL_OPTIONS, module);
            for (const rule of rules) {
                const triggerFunction = async () => {
                    const requestId = decorator_1.Utils.randomUUID();
                    const ctx = this.app.createAnonymousContext({
                        taskInfo: {
                            type: 'LocalTask',
                            id: requestId,
                    ...
                    try {
                        ...
                        await decorator_1.Utils.toAsyncFunction(rule.value.bind(service))();
                    }
                    catch (err) {
                        logger.error(err);
                    }
    ...
    async loadQueue() {
        const modules = (0, decorator_1.listModule)(decorator_1.MODULE_TASK_QUEUE_KEY);
        const taskConfig = this.configService.getConfiguration('task');
        const config = JSON.parse(JSON.stringify(taskConfig));
        const concurrency = config.concurrency || 1;
        delete config.defaultJobOptions.repeat;
        for (const module of modules) {
            const rule = (0, decorator_1.getClassMetadata)(decorator_1.MODULE_TASK_QUEUE_OPTIONS, module);
            ...
            queue.process(concurrency, async (job) => {
                ...
                try {
                    ...
                    const service = await ctx.requestContext.getAsync(module);
                    await decorator_1.Utils.toAsyncFunction(service.execute.bind(service))(job.data, job);
                }
                catch (e) {
                    logger.error(`${e.stack}`);
                }
