...
let CoolRpc = class CoolRpc {
    async init() {
        ...
        if (!((_a = this.rpcConfig) === null || _a === void 0 ? void 0 : _a.name)) {
            throw new core_1.CoolCoreException('cool.rpc.name config is require and every service name must be unique');
        }
        let redisConfig;
        if (!((_b = this.rpcConfig) === null || _b === void 0 ? void 0 : _b.redis) && !((_c = this.coolConfig) === null || _c === void 0 ? void 0 : _c.redis)) {
            throw new core_1.CoolCoreException('cool.rpc.redis or cool.redis is require');
        }
        ...
        this.broker = new moleculer_1.ServiceBroker({
            nodeID: `${this.rpcConfig.name}-${uuid_1.v1()}`,
            transporter,
            ...this.rpcConfig,
        });
    ...
    async getEvents() {
        const allEvents = {};
        const modules = decorator_1.listModule(event_1.COOL_RPC_EVENT_KEY);
        for (const module of modules) {
            const moduleInstance = await this.app
                .getApplicationContext()
                .getAsync(module);
            moduleInstance['broker'] = this.broker;
            const events = decorator_1.getClassMetadata(handler_1.COOL_RPC_EVENT_HANDLER_KEY, module);
        ...
        this.broker.createService({
            ...
            actions: {
                async call(ctx) {
                    ...
                    if (!target) {
                        throw new core_1.CoolValidateException('找不到服务');
                    }
                    const curdOption = decorator_1.getClassMetadata(rpc_1.MOLECYLER_KEY, target);
                    const cls = await _this.app.getApplicationContext().getAsync(service);
                    ...
                    if (['add', 'delete', 'update', 'page', 'info', 'list'].includes(method)) {
                        if (!curdOption.method.includes(method)) {
                            throw new core_1.CoolValidateException('方法不存在');
                        }
                    }
    ...
    async initService() {
        // 获得所有的service
        this.cruds = decorator_1.listModule(rpc_1.MOLECYLER_KEY);
        for (const crud of this.cruds) {
            const curdOption = decorator_1.getClassMetadata(rpc_1.MOLECYLER_KEY, crud);
